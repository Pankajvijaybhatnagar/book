...
const Events = () => {
  ...
  const handleCategoryClick = (selectedCategory) => {
    navigate(category === selectedCategory ? "/events" : `/events/${encodeURIComponent(selectedCategory)}`);
  };
  ...
  return (
    <div className="container mx-auto p-5">
      ...
      <div className="flex flex-col sm:flex-row items-start gap-8 mt-5">
        ...
        {/* Filter List */}
        <div className={`flex-col gap-4 text-sm text-blue-600 ${category ? "flex" : "hidden sm:flex"}`}>
          {categoryList.map((cat) => (
            <p
              key={cat}
              onClick={() => handleCategoryClick(cat)}
              className={`w-[94vw] sm:w-auto pl-4 py-2 pr-16 border border-primary rounded cursor-pointer transition-all ${
                category === cat ? "bg-orange-500 text-white font-semibold" : "hover:bg-orange-100"
              }`}
            >
              {cat}
            </p>
